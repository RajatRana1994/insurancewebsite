<!DOCTYPE html>
<html lang="en-US">
   <head>
        <?php include("../head.php"); ?>
    </head>
    <body class="product-page">
        <header class="header header-light">
            <?php include("../header.php"); ?>
        </header>
        <section class="banner inner-page">
            <div class="banner-content">
                <div class="banner-inner">
                    <div class="typed_wrap">
                        <h1 class="banner-title">Accident Forgiveness</h1>
                        <h4 class="subtitle">Home / Products / Auto Insurance / Accident Forgiveness</h4>
                    </div>
                </div>
            </div>
        </section>

        <div class="svg-wrapper bg-white">
            <svg version="1.1" id="Layer_1" class="footer-top" x="0px" y="0px" width="1920px" height="58.223px" viewBox="0 0 1920 58.223" enable-background="new 0 0 1920 58.223" xml:space="preserve"><path fill="#f9f8f4" d="M1920,233.223H0C0,233.223,1600.488,364.223,1920,233.223z"/><path fill="#f9f8f4" d="M0,58.223h1920C1920,58.223,319.512-72.777,0,58.223z"/></svg>
        </div>

        <div class="section product-data bg-light">
            <div class="container">
                <!--Product Content Wrapper-->
                <div class="product-content-wrapper">
                    <div class="row justify-content-between">
                        <div class="col-12 col-md-8">
                            <div class="section-header">
                                <h2 class="section-title">Don’t Let One Accident Hurt Your Record.</h2>
                                <p>Accident Forgiveness in Woodbridge NJ & New York City NY
                                </p>
                            </div>
                            <p>We understand that accidents happen. On the road, there are many situations that are outside of your control and sometimes cannot be avoided. At Defy Insurance, we don’t want an accident to hurt your driving record, or even cause your rates to go up. With the right coverage, you can be forgiven for your first chargeable accident through Accident Forgiveness.</p>
                            <p>Under Accident Forgiveness, if you’re involved in an accident, your rates won’t increase – even if the accident is your fault. This applies only one time per policy, and once it’s been used, you’ll be eligible to receive it again three years later.</p>
                            <p>Contact Defy Insurance today to find out how you can receive the benefits of Accident Forgiveness for your Auto Insurance policy.</p>
                        </div>
                        <div class="col-12 col-md-3">
                            <div class="card p-4">
                                <a href="<?php echo $base_url . 'get-quote' ?>" class="btn btn-block btn-primary get-a-quote-btn">Get a Quote</a>
                            </div>
                        </div>
                    </div>
                </div>
                <!--//Product Content Wrapper-->
            </div>
        </div>

        <!-- Footer Starts -->
        <?php include("../footer.php"); ?>
        <!-- Footer Ends -->
    </body>
</html>